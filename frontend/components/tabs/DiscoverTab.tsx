...
export type DiscoverTabProps = {
  /** Load feed only when the Discover tab is visible (same pattern as For You / Following). */
  isTabActive?: boolean;
};

export default function DiscoverTab({ isTabActive = true }: DiscoverTabProps) {
  const { t } = useLanguage();
  ...
  /** Last category that finished a successful load (`undefined` = never). */
  const loadedCategoryRef = useRef<string | null | undefined>(undefined);
  ...
  const loadDiscover = useCallback(async (category: string | null) => {
    ...
    try {
      ...
      const initialCards = res.items.map((item) => mapFeedItemToCard(item, null));
      setCards(initialCards);
      setHasLoadedOnce(true);
      loadedCategoryRef.current = category;

      void prefetchDiscoverCovers(res.items, {
        concurrency: network.coverConcurrency,
        deadlineMs: Math.min(network.coverDeadlineMs, 4_000),
      })
        .then((coverById) => {
          if (generation !== loadGeneration.current) return;
          setCards(
            res.items.map((item) =>
              mapFeedItemToCard(item, coverById.get(item.id) ?? null),
            ),
          );
        })
        .catch(() => {
          /* Cards already shown without covers. */
        });
    } catch (e) {
      if (generation !== loadGeneration.current) return;
      setCards([]);
  ...
  useEffect(() => {
    ...
    if (loadedCategoryRef.current === activeDiscoverCategory) return;
    void loadDiscover(activeDiscoverCategory);
  }, [activeDiscoverCategory, isTabActive, loadDiscover]);
        ...
        {feedLoading && cards.length === 0 ? (
          ...
          <View style={styles.emptyCategoryWrap}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                loadedCategoryRef.current = undefined;
                void loadDiscover(activeDiscoverCategory);
              }}
              accessibilityRole="button"
            >
