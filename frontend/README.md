...
**safick** is a social commerce-driven application that enables users to discover new products, connect with sellers, and shop—while providing an entertaining experience. It is built with React Native and Expo and combines shopping, live streaming, and social features.

## Market Context

**safick** is designed specifically for the African market, starting with **Cameroon**. The platform solves a critical pain point: sellers currently share product videos via WhatsApp to their contact lists, who then share to their stories - a fragmented and inefficient process.
...
safick serves two distinct user types:
...
- sellers can also host events and go live
...
safick is built as three independent services that communicate over HTTP and WebSocket:
...
safick includes comprehensive AI-powered features built entirely with open-source, self-hosted solutions (no paid API dependencies).
...
```env
# Database
DATABASE_URL=postgresql://user:pass@localhost:5432/safick
...
safick is built by a team of 4, each owning a distinct area of the product.
...
**Responsibilities:**
- AI/ML backend development (Python / FastAPI shared with Lead Developer)
