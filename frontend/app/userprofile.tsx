...
/** Solid banner when there is no cover image (e.g. logo/business photo). Tie to profile accent / theme from API later. */
const PROFILE_COVER_FALLBACK_COLOR = "#929292";

/**
 * Optional profile banner: business logo, storefront, brand graphic, or `null` for solid fallback color above.
 * Later: `{ uri: remoteUrl }` from upload or API.
 */
const USER_COVER_IMAGE: ImageSourcePropType | null = null;

const TRUST_METRICS = {
  productSatisfaction: 94,
  replyTime: "10 min",
  replyWithin30MinRate: 89,
  onTimeDelivery: 96,
} as const;
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* Header: optional cover image, else solid profile/brand color */}
        <View style={styles.headerWrapper}>
          {USER_COVER_IMAGE != null ? (
            <>
              <ImageBackground source={USER_COVER_IMAGE} style={styles.headerBackground} resizeMode="cover" />
              <View style={styles.headerOverlay} />
            </>
          ) : (
            <View style={[styles.headerBackground, styles.headerBackgroundFallback]} />
          )}
          ...
          {/* Profile: avatar, name, join date, location, then Follow + Message buttons close below */}
