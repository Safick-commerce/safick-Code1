...
**The critical insight about your app model:** wispaCart is a **message-to-buy marketplace**, not a traditional e-commerce app. There is no cart, no checkout, no payment processing. The entire transaction happens through **chat**. This means **chat is your most important backend feature** -- not a secondary one.
...
Create `backend/` as a sibling to `wispaCart/` in the workspace root:
...
### Things most tutorials won't tell you (specific to wispaCart):

1. **Chat is not a feature, it's the product.** In most e-commerce apps, chat is secondary. In wispaCart, if chat breaks, the entire business breaks. Invest heavily in chat reliability, message delivery guarantees, and offline support. Use a message queue (Redis streams or SQS) to ensure no messages are lost.
...
8. **WhatsApp sharing is a growth feature, not a nice-to-have.** Your users already share products on WhatsApp. Build deep links (using Expo Linking + a simple web redirect page) so when someone shares a product link on WhatsApp, clicking it opens the app to that product. This is your #1 organic growth channel.
...
| Push       | Firebase Cloud Messaging | Free, works on both Android and iOS                                  |
