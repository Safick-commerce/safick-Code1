...
/** Profile columns consumed by mapProductToFeedItem / ForYouFeedSellerResponse. */
const feedSellerProfileSelect = {
  username: true,
  display_name: true,
  full_name: true,
  avatar_url: true,
...
const discoverFeedProductInclude = {
  profiles: { select: feedSellerProfileSelect },
} as const;
...
type DiscoverProductWithSeller = Prisma.productsGetPayload<{
  include: typeof discoverFeedProductInclude;
}>;
...
async function buildDiscoverFeed(
  ...
  const rows = await prisma.products.findMany({
    ...
    orderBy: feedOrderBy(),
    take: limit + 1,
    include: discoverFeedProductInclude,
  });
...
function mapProductToFeedItem(
  row: ProductWithSeller | DiscoverProductWithSeller,
): ForYouFeedItemResponse {
